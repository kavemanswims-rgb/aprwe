...
def import_ytd_adjustments(uploaded_file, replace_existing=True):
    ...
    with db_connect() as conn:
        ...
        for idx, row in df.iterrows():
            name = str(first_available(row, ["employee_name", "name", "employee", "driver", "worker"], "")).strip()
            ssn_last4 = normalize_ssn_last4(first_available(row, ["ssn_last4", "last_4", "last4", "social_sec_id", "social_security_last4", "social_security", "ssn", "employee_ssn", "tax_id_last4"], ""))
            matched = None
            if ssn_last4 and ssn_last4 in employee_by_ssn:
            ...
            if matched is not None:
                ...
                skipped.append(f"Row {idx + 2}: missing employee name/SSN last 4")
                continue

            ytd_gross = safe_num(first_available(row, ["ytd_gross", "gross_ytd", "gross_pay_ytd", "gross", "gross_pay", "year_to_date_gross", "ytd_pay", "pay_ytd", "total_gross", "ytd_total_gross"], 0))
            ytd_federal = safe_num(first_available(row, ["ytd_federal", "federal_ytd", "federal_tax_ytd", "federal_tax", "fed_tax", "fed_ytd", "federal_withholding", "federal_withholding_ytd"], 0))
            ytd_ss = safe_num(first_available(row, ["ytd_ss", "ytd_social_security", "social_security_ytd", "social_security", "ss_ytd", "social_security_tax", "social_security_tax_ytd", "fica_ss", "fica_social_security"], 0))
            ytd_medicare = safe_num(first_available(row, ["ytd_medicare", "medicare_ytd", "medicare", "medicare_tax", "medicare_tax_ytd", "fica_medicare"], 0))
            ytd_virginia = safe_num(first_available(row, ["ytd_virginia", "virginia_ytd", "state_withholding_ytd", "state_withholding", "virginia_tax_ytd", "state_tax_ytd", "va_tax", "va_tax_ytd", "virginia_withholding", "state"], 0))
            ytd_deductions = safe_num(first_available(row, ["ytd_deductions", "deductions_ytd", "total_deductions_ytd", "deductions", "ytd_total_deductions", "total_deductions"], 0))
            if ytd_deductions == 0:
                ytd_deductions = ytd_federal + ytd_ss + ytd_medicare + ytd_virginia
            ytd_net = safe_num(first_available(row, ["ytd_net", "net_ytd", "net_pay_ytd", "net_pay", "year_to_date_net", "ytd_net_pay", "take_home_ytd", "total_net"], 0))
            ...
            """, (
                employee_id, employee_name, ssn_last4, ytd_gross, ytd_deductions, ytd_net,
                ytd_federal, ytd_ss, ytd_medicare, ytd_virginia, datetime.now().isoformat(timespec="seconds"),
            ))
            inserted += 1
        conn.commit()
...
with tab_employees:
    ...
    ytd_saved = read_df("SELECT employee_name, ssn_last4, ytd_gross, ytd_deductions, ytd_net, ytd_federal, ytd_ss, ytd_medicare, ytd_virginia, imported_at FROM ytd_adjustments ORDER BY employee_name")
    if not ytd_saved.empty:
        st.dataframe(df_money_columns(ytd_saved, ["ytd_gross", "ytd_deductions", "ytd_net", "ytd_federal", "ytd_ss", "ytd_medicare", "ytd_virginia"]), width="stretch", hide_index=True)
        if st.button("Clear Imported YTD Totals"):
            clear_ytd_adjustments()
            st.success("Imported YTD totals cleared.")
            st.rerun()
